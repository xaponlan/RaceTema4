...
```{r}
cran2%>%
  arrange(country,-r_version,ip_id)
```
...
top_counts <- cran %>%
  ...
  filter(count > quantile(count, probs = 0.99, na.rm = TRUE))
...
cran %>%
  ...
  filter(total_traffic > quantile(total_traffic, probs = 0.90, na.rm = TRUE)) %>%  
  arrange(desc(total_traffic)) %>%  
  ggplot(aes(x = reorder(country, -total_traffic), y = total_traffic))+geom_point()
...
```{r}
library(tidyverse)
load(file = "datostidyr.Rdata")
# Las variables son grade, male y female

tidy_students <- students %>%
  pivot_longer(cols = 2:3, names_to = "sex", values_to = "count")
tidy_students
...
```{r}
tidy_students2 <- students2 %>%
  pivot_longer(cols = c(male_1,female_1,male_2, female_2), names_to = c("sex","class"), names_sep = "_", values_to = "count")
tidy_students2
...
tidy_students3 <- students3 %>%
  gather(key = "class", value = "score", 3:7, na.rm = TRUE) %>%
...
students_cal <- bind_rows(failed, passed)
students_cal
```
...
```{r}
tidy_sat <- sat %>%
  pivot_longer(cols = c(read_male,read_fem, math_male,math_fem,write_male,write_fem), names_to = c("part","sex"), names_sep = "_", values_to = "count") %>%
  select(score_range, part, sex, count)
...
```{r}
ggplot(tidy_sat, aes(x = score_range, y = prop, colour = sex)) +
  geom_point() + 
  facet_wrap(.~part) + 
...
llegada_ny <- salida + hour(15) + minute(50)

llegada_hk <- with_tz(llegada_ny,tzone = "Asia/Hong_Kong")

ultima_vez <- dmy("17 June 2008",tz = "Asia/Singapore")
tiempo_transcurrido <- interval(ultima_vez, now()) %>% as.period()
tiempo_transcurrido
#Hace 16 años 8 meses y 24 días
```
